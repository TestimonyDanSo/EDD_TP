...
package edd_tp1.jreadfilecategory_0_1;
...
/**
 * Job: jReadFileCategory Purpose: <br>
 * Description: <br>
 * 
 ...
 * @status
 */
public class jReadFileCategory implements TalendJob {
	static {
		System.setProperty("TalendJob.log", "jReadFileCategory.log");
	}

	private static org.apache.logging.log4j.Logger log = org.apache.logging.log4j.LogManager
			.getLogger(jReadFileCategory.class);
	...
	private final String jobName = "jReadFileCategory";
	...
	private class TalendException extends Exception {
		...
		public void printStackTrace() {
			...
			if (!(e instanceof TalendException)) {
				jReadFileCategory.this.exception = e;
			}
			if (!(e instanceof TalendException)) {
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(jReadFileCategory.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
	...
	public static class outlineStruct implements routines.system.IPersistableRow<outlineStruct> {
		final static byte[] commonByteArrayLock_EDD_TP1_jReadFileCategory = new byte[0];
		static byte[] commonByteArray_EDD_TP1_jReadFileCategory = new byte[0];
		...
		public boolean equals(Object obj) {
			...
			if (getClass() != obj.getClass())
				return false;
			final outlineStruct other = (outlineStruct) obj;
		...
		public void copyDataTo(outlineStruct other) {
		...
		public void copyKeysDataTo(outlineStruct other) {
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_EDD_TP1_jReadFileCategory.length) {
					if (length < 1024 && commonByteArray_EDD_TP1_jReadFileCategory.length == 0) {
						commonByteArray_EDD_TP1_jReadFileCategory = new byte[1024];
					} else {
						commonByteArray_EDD_TP1_jReadFileCategory = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_EDD_TP1_jReadFileCategory, 0, length);
				strReturn = new String(commonByteArray_EDD_TP1_jReadFileCategory, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException {
			...
			if (length == -1) {
				...
				unmarshaller.readFully(commonByteArray_EDD_TP1_jReadFileCategory, 0, length);
				strReturn = new String(commonByteArray_EDD_TP1_jReadFileCategory, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_EDD_TP1_jReadFileCategory) {
		 ...
		 * Compare keys
		 */
		public int compareTo(outlineStruct other) {
	...
	public void tFileInputDelimited_1Process(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		mdc("tFileInputDelimited_1", "h8Ru9W_");
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				outlineStruct outline = new outlineStruct();
				...
				runStat.updateStatAndLog(execStat, enableLogStash, resourceMap, iterateId, 0, 0, "outline");
...
// # Outputs initialization
				int count_outline_tMap_1 = 0;

				outlineStruct outline_tmp = new outlineStruct();
// ###############################
...
// # Output table : 'outline'
								count_outline_tMap_1++;

								outline_tmp.code_category = row1.code_category;
								outline_tmp.description_category = row1.description_category;
								outline = outline_tmp;
								log.debug("tMap_1 - Outputting the record " + count_outline_tMap_1
										+ " of the output table 'outline'.");
...
// Start of branch "outline"
							if (outline != null) {
								...
								if (runStat.update(execStat, enableLogStash, iterateId, 1, 1
										...
										, "outline", "tMap_1", "tMap_1", "tMap", "tLogRow_1", "tLogRow_1", "tLogRow"
								...
								if (log.isTraceEnabled()) {
									log.trace("outline - " + (outline == null ? "" : outline.toLogString()));
								}
...
///////////////////////		
								...
								if (outline.code_category != null) { //
									row_tLogRow_1[0] = String.valueOf(outline.code_category);

								} //

								if (outline.description_category != null) { //
									row_tLogRow_1[1] = String.valueOf(outline.description_category);
...
// ###############################      
				log.debug("tMap_1 - Written records count in the table 'outline': " + count_outline_tMap_1 + ".");
...
///////////////////////    			
				...
				if (runStat.updateStatAndLog(execStat, enableLogStash, resourceMap, iterateId, "outline", 2, 0,
	...
	public static void main(String[] args) {
		final jReadFileCategory jReadFileCategoryClass = new jReadFileCategory();

		int exitCode = jReadFileCategoryClass.runJobInTOS(args);
		if (exitCode == 0) {
			log.info("TalendJob: 'jReadFileCategory' - Done.");
		}
	...
	public int runJobInTOS(String[] args) {
		...
		getjobInfo();
		log.info("TalendJob: 'jReadFileCategory' - Start.");
		...
		org.slf4j.MDC.put("_jobRepositoryId", "_Qb1OwJk0EfCtKshYtxjGzg");
		org.slf4j.MDC.put("_compiledAtTimestamp", "2025-10-07T11:42:13.859701Z");
		...
		try {
			...
			java.io.InputStream inContext = jReadFileCategory.class.getClassLoader()
					.getResourceAsStream("edd_tp1/jreadfilecategory_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = jReadFileCategory.class.getClassLoader()
		...
		log.info("TalendJob: 'jReadFileCategory' - Started.");
		...
		if (false) {
			System.out.println(
					(endUsedMemory - startUsedMemory) + " bytes memory increase when running : jReadFileCategory");
		}
		if (enableLogStash) {
		...
		log.info("TalendJob: 'jReadFileCategory' - Finished - status: " + status + " returnCode: " + returnCode);
...
/************************************************************************************************
 * 91779 characters generated by Talend Cloud API Services Platform on the 7
 * octobre 2025 à 13:42:13 CEST
 ************************************************************************************************/